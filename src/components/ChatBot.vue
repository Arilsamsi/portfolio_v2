...
<template>
  ...
  <div
    ...
    >
      ...
      <section class="chats h-[368px] overflow-y-auto p-4 space-y-4 dark:bg-black/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800" ref="chatContainer">
        <div v-for="(message, index) in chatHistory" :key="index" :class="['message', message.type === 'user' ? 'message--outgoing' : 'message--incoming', message.isLoading ? 'message--loading' : '', message.isError ? 'message--error' : '']">
          <div class="message__content">
            <img class="message__avatar" :src="message.type === 'user' ? userAvatar : geminiAvatar" :alt="message.type + ' avatar'">
          ...
          <span v-if="!message.isLoading && message.type === 'bot'" @click="copyMessageToClipboard(message.text, $event)" class="message__icon"><i class='bx bx-copy-alt'></i></span>
        </div>
      </section>
  
      <section class="prompt text-white border-t border-t bg-blue-500 rounded md:w-[400px]  dark:bg-gray-900/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800">
        <form @submit.prevent="handleSendMessage" class="prompt__form">
          <div class="prompt__input-wrapper">
            <input type="text" v-model="userMessage" placeholder="Enter a prompt here" class="prompt__form-input w-[400px] md:w-[400px] text-white focus:border-blue-900 dark:bg-gray-900/80 backdrop-blur-sm border-b border-gray-200 dark:border-gray-800" required>
            <button type="submit" class="prompt__form-button" id="sendButton">
              <i class='bx bx-send'></i>
            </button>
            <button type="button" @click="clearChat" class="prompt__form-button" id="deleteButton">
